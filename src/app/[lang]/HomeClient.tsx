...
import VoiceResultSheet from "@/components/ui/VoiceResultSheet";
import { speak, buildDynamicResponse, type VoiceIntent } from "@/lib/voiceAssistant";
...
const nearbyEssentialsConfig = [
  { key: "food", category: "food", emoji: "🍲", fallbackDist: "300m", bg: "bg-orange-50", border: "border-orange-200", text: "text-orange-800", href: "facilities?type=food" },
  { key: "toilet", category: "toilet", emoji: "🚻", fallbackDist: "150m", bg: "bg-teal-50", border: "border-teal-200", text: "text-teal-800", href: "facilities?type=toilet" },
  { key: "water", category: "water", emoji: "💧", fallbackDist: "200m", bg: "bg-sky-50", border: "border-sky-200", text: "text-sky-800", href: "facilities?type=water" },
  { key: "medical", category: "medical", emoji: "🏥", fallbackDist: "500m", bg: "bg-rose-50", border: "border-rose-200", text: "text-rose-800", href: "facilities?type=medical" },
];
...
export default function HomeClient({ lang }: { lang: string }) {
  ...
  // Calculate real distances if GPS is available
  ...
  // Sort by real distance, pick nearest and least crowded (different ghats)
  const sorted = [...ghatsWithDist].sort((a, b) => a.realDist - b.realDist);
  const nearest = sorted[0] || null;
  const leastCrowded = sorted.length > 1
    ? sorted
        .filter((g) => g.id !== nearest?.id)
        .sort((a, b) => {
          const crowdOrder = { low: 0, medium: 1, high: 2 };
          const diff = crowdOrder[a.crowd] - crowdOrder[b.crowd];
          return diff !== 0 ? diff : a.realDist - b.realDist;
        })[0]
    : null;

  // Calculate nearest facility distance for each essential category
  const nearbyEssentials = nearbyEssentialsConfig.map((item) => {
    const categoryFacilities = facilityLocations.filter((f) => f.category === item.category);
    if (userLoc && categoryFacilities.length > 0) {
      const nearest = categoryFacilities
        .map((f) => ({ ...f, dist: getDistanceKm(userLoc.lat, userLoc.lng, f.lat, f.lng) }))
        .sort((a, b) => a.dist - b.dist)[0];
      return { ...item, distance: formatDistance(nearest.dist) };
    }
    return { ...item, distance: item.fallbackDist };
  });
    ...
    <div className="flex-1 flex flex-col overflow-x-hidden bg-godavari-50">
        ...
        <div className="px-4 space-y-3 -mt-4 relative z-10">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            >
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-1">
                  <span className={clsx("w-2 h-2 rounded-full", nearest?.crowd === "high" ? "bg-red-500" : nearest?.crowd === "medium" ? "bg-amber-500" : "bg-emerald-500")} />
                  <span className={clsx("text-[9px] font-bold", nearest?.crowd === "high" ? "text-red-700" : nearest?.crowd === "medium" ? "text-amber-700" : "text-emerald-700")}>
                    {nearest?.crowd === "high" ? t.crowdHigh : nearest?.crowd === "medium" ? t.crowdMed : t.crowdLow}
                  </span>
                </div>
              </div>
              ...
              <p className="text-sm font-bold text-godavari-950 leading-tight mt-0.5">
                {nearest ? nearest.name[l] : "..."}
              </p>
              <p className="text-[11px] text-godavari-400 mt-1">{nearest?.realDistLabel || "..."} →</p>
            </Link>
            ...
            >
              ...
              <p className="text-sm font-bold text-godavari-950 leading-tight mt-0.5">
                {leastCrowded ? leastCrowded.name[l] : "..."}
              </p>
              <p className="text-[11px] text-godavari-400 mt-1">{leastCrowded?.realDistLabel || "..."} →</p>
            </Link>
          </div>
          ...
          {latestAlert && (
            <Link
              href={`/${lang}/alerts`}
              className="block rounded-xl px-4 py-3 bg-amber-50 border border-amber-200 active:scale-[0.98] transition-transform"
      ...
      <VoiceModal
        ...
        onNavigate={(route, intent) => {
          ...
          // Build dynamic response with real GPS distances
          const currentLang = (lang as Lang) || "te";
          const facilitiesWithDist = facilityLocations.map((f) => ({
            ...f,
            realDist: userLoc ? getDistanceKm(userLoc.lat, userLoc.lng, f.lat, f.lng) : 999,
            realDistLabel: userLoc ? formatDistance(getDistanceKm(userLoc.lat, userLoc.lng, f.lat, f.lng)) : (f.distance || ""),
          }));
          const { text, fallback } = buildDynamicResponse(intent, ghatsWithDist, facilitiesWithDist, currentLang);
          speak(text, currentLang, fallback);
